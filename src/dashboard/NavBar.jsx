...
const NavBar = () => {
  ...
  // Use Zustand store for isMobile and toggleMobileMenu
  const { isMobile, toggleMobileMenu, isMobileMenuOpen } = useSidebarStore();
  ...
  return (
    <>
      <header className="sticky absolute top-0 left-0 w-full h-16 bg-[#1A114A] p-4 z-30 round">
        <div className="flex flex-row justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            {isMobile && (
              <button 
                ...
                onClick={toggleMobileMenu} // Use toggleMobileMenu from Zustand store
            ...
            <h1 className="text-sm xl:text-2xl lg:text-xl md:text-lg font-bold text-white">DATACOVE AI</h1>
          </div>
          ...
          {isDashboardHomePage ? (
            ...
            <Link to={`/dashboard/${user?.name}`} className="bg-[#382fa3] text-white px-2 py-1 md:px-4 md:py-2 rounded text-xs md:text-sm">
              Back to Dashboard
            </Link>
          )}
        </div>
