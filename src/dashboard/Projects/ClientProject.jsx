...
import DocsForClient from "../Table/DocsForClient.jsx";
...
const ClientsProject = () => {
  ...
  // Initialize selectedClient with localStorage
  ...
  // Fetch clients from user's clients array
  const fetchClients = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3000/api/v1/dashboard/getClients",
        { 
      ...
      // If we have a selected client in localStorage, make sure it exists in the fetched clients
      if (selectedClient) {
        const clientExists = clients.some(c => c._id === selectedClient._id);
        if (!clientExists) {
  ...
  const uploadFiles = async () => {
    ...
    formData.append("clientId", selectedClient.clientId || selectedClient._id);
    formData.append("consultantId", user._id);
    
    try {
      const response = await axios.post(
        "http://localhost:3000/api/v1/upload/shareWithClient",
        formData,
        {
          withCredentials: true,
          headers: {
      ...
      toast.error(error.response?.data?.error || "Failed to share files");
    } finally {
  ...
  return (
    <div className="space-y-6 p-6 bg-[#2E1D6A]">
      ...
      {/* Display file upload and DocsForClient when a client is selected */}
      {selectedClient && (
        <div className="mt-6">
          <div className="flex flex-col lg:flex-row justify-between lg:items-center mb-6 gap-4">
            <h2 className="text-2xl font-bold text-white">
              Share with {selectedClient.name}
            </h2>
