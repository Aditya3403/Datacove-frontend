    ...
    <div className="flex flex-col h-full w-full bg-[#1A114A] text-white">
      ...
      {/* <div className="flex flex-1 overflow-hidden"> */}
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="flex justify-between items-center mb-8">
            ...
            <button 
              className="bg-[#6c21a8] hover:bg-purple-700 px-4 py-2 rounded-md flex items-center"
          ...
          <div className="space-y-4">
            {sortedWorkflows.map(workflow => (
              <div key={workflow.id} className="bg-indigo-900 rounded-lg overflow-hidden shadow-lg hover:ring-1 hover:ring-purple-500 transition-all">
                <div className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex space-x-2">
                      <span className={`px-2 py-1 text-xs rounded-full ${getCategoryInfo(workflow.category).color}`}>
                        {getCategoryInfo(workflow.category).label}
                      </span>
                      <span className={`px-2 py-1 text-xs rounded-full ${getStatusInfo(workflow.status).color}`}>
                        {getStatusInfo(workflow.status).label}
                      </span>
                    </div>
                  </div>
                  
                  <p className="text-indigo-300 mb-4">{workflow.description}</p>
                  
                  <div className="flex flex-wrap gap-2 mb-4">
                    {workflow.components.map((component, index) => (
                      <span key={index} className="px-2 py-1 text-xs bg-indigo-800 rounded-md">
                        {component}
                      </span>
                    ))}
                  </div>
          ...
          {filteredWorkflows.length === 0 && (
            <div className="bg-indigo-900 rounded-lg p-8 text-center">
              ...
              <p className="text-indigo-300 mb-6">Create a new workflow or adjust your search criteria.</p>
              <button 
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-indigo-900 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  <button 
                    className="px-4 py-2 border border-indigo-700 rounded-md hover:bg-indigo-800"
                    onClick={() => setShowCreateModal(false)}
                  >
                    Cancel
                  </button>
